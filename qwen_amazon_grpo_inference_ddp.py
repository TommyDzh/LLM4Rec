...
def main(args):
    ...
    dataset_test = pd.read_parquet(os.path.join(args.data_path, args.data_test))
    ...
    for row in dataset_test.itertuples():
        uid = row.extra_info['user_id']
        tid = row.extra_info['parent_asin']
        test_id = f"{uid}-{tid}"  
    ...
    if accelerator.is_main_process:
        ...
        if "SFT" not in model_name:
            output_dir = os.path.join("./results/logits", f"Zero-{dataset_name}-{model_name}")
        else:
            output_dir = os.path.join("./results/logits", f"{model_name}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model", type=str, default="/mnt/hdfs/zhihao/AmazonReviews/ckpt/GRPO_Books_max@30-long@True_step360")
    parser.add_argument("--data_path", type=str, default="/mnt/hdfs/zhihao/AmazonReviews/Books/verl/max@30-long@True", help="Path to training jsonl")
    parser.add_argument("--data_test", type=str, default="test.parquet", help="Path to training jsonl")
